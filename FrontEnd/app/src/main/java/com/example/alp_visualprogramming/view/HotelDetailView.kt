...
@Composable
fun HotelDetailView(location: Location, navController: NavController, journeyId: Int, journeyViewModel: JourneyViewModel) {
                    ...
                    modifier = Modifier
                        ...
                        .clickable {
                            journeyViewModel.viewModelScope.launch {
                                try {
                                    // Replace with your actual token and journey ID
                                    val token = "f57031c8-1c62-4bea-947b-4239db58e31c" // Fetch dynamically
                                    val journeyId = journeyId // Use the journeyId passed to the HotelDetailView

                                    // Step 1: Call getOrCreateAccommodation
                                    val accommodationResponse = journeyViewModel.getOrCreateAccommodation(
                                        token = token,
                                        accommodationRequest = AccommodationRequest(
                                            place_id = location.place_id,
                                            name = location.name,
                                            address = location.address ?: "No address",
                                            location_image = "default_image_url", // Correct field name
                                            place_api = "geoapify",
                                            categories = location.categories,
                                            opening_hours = location.openingHours,
                                            website = location.website,
                                            phone = location.phone,
                                            cost = totalCost.value.toDouble(), // Convert BigDecimal to Double
                                            people = occupants.value
                                        )
                                    )

                                    // Step 2: Update the journey with the accommodation details
                                    journeyViewModel.updateJourneyAccommodation(
                                        token = token,
                                        itineraryDestinationId = journeyId,
                                        accommodationRequest = AccommodationRequest(
                                            id = accommodationResponse.id,
                                            place_id = accommodationResponse.place_id,
                                            name = accommodationResponse.name,
                                            address = accommodationResponse.address,
                                            location_image = accommodationResponse.location_image,
                                            place_api = accommodationResponse.place_api,
                                            categories = accommodationResponse.categories,
                                            cost = accommodationResponse.cost,
                                            people = accommodationResponse.people,
                                            opening_hours = accommodationResponse.opening_hours,
                                            website = accommodationResponse.website,
                                            phone = accommodationResponse.phone
                                        )
                                    )

                                    // Step 3: Navigate back or show success message
                                    navController.popBackStack()
                                } catch (e: Exception) {
                                    // Handle errors gracefully
                                    Log.e("HotelDetailView", "Error: ${e.message}")
                                }
                            }
                        },
